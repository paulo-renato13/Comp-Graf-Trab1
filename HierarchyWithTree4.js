 ...
 * 
 * Adapted to implement a Turbine using a Matrix Stack.
 * 
 */
...
/*
  Increment variable for rotor rotation
*/
var increment = 1;

/**
 * Transformation matrix that is the root of the objects in the scene.
 * @type {Matrix4}
 */
var shaftMatrix = new Matrix4()
  .setTranslate(0, -5, 0)
  .rotate(joint.shaft, 0, 1, 0);
...
var generatorBlobMatrixLocal = new Matrix4().setScale(2, 1.2, 3.2);

var rotorMatrixLocal = new Matrix4().setScale(1.5, 1.5, 1.5);
var rotorBlobMatrixLocal = new Matrix4().setScale(1, 1, 1);
...
var cube = (() => {
  ...
  for (var i = 0; i < 36; ++i) {
    ...
    for (var j = 0; j < 4; ++j) {
      colorsArray.push(rawColors[4 + j]);
    }
...
function handleKeyPress(event) {
  ...
  switch (ch) {
    case "t":
      joint.shaft += 15; //Clockwise
      shaftMatrix.setTranslate(0, -5, 0).rotate(joint.shaft, 0, 1, 0);
      break;
    case "T":
      joint.shaft -= 15; //Anti-Clockwise
      shaftMatrix.setTranslate(0, -5, 0).rotate(joint.shaft, 0, 1, 0);
      break;
    case "g":
      joint.generator += 15; // rotate generator clockwise 
      var currentgeneratorRot = new Matrix4()
        .setTranslate(0, 0, 0)
        .rotate(joint.generator, 0, 1, 0)
        .translate(0, 0, 0);
      generatorMatrix.setTranslate(0, 11, 0).multiply(currentgeneratorRot);
      break;
    case "G":
      joint.generator -= 15; // rotate generator anti-clockwise 
      var currentgeneratorRot = new Matrix4()
        .setTranslate(0, 0, 0)
        .rotate(joint.generator, 0, 1, 0)
        .translate(0, 0, 0);
      ...
      break;
    case "r":
      increment = -increment; // reverse blade rotation
      break;
    case "R":
      increment = -increment; // reverse blade rotation
      break;
    case "b":
      increment = increment*1.25; // slow down blade rotation
      break;
    case "B":
      increment = increment/1.25; // speed up blade rotation
      break;
    case "ArrowUp":
      // Up pressed
...
function draw(useRotator = true) {
  ...
  // generator blob relative to generator
...
window.addEventListener("load", (event) => {
  ...
  (function animate() {
    ...
    joint.rotor += increment;
    // Rotor rotation animation
    var currentrotor = new Matrix4()
      .setTranslate(0, 0, 0)
      .rotate(joint.rotor, 0, 0, 1)
      .translate(0, 0, 0);
    rotorMatrix.setTranslate(0, 0, 3).multiply(currentrotor);
